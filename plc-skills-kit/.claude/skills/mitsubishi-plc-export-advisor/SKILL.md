---
name: plc-export-advisor
description: Plan what to export from GX Works2/GX Works3 projects for Git version control and documentation. Use when the user asks how to export PLC project data, prepare raw files, or document GX Works projects. 協助使用者規劃三菱 PLC 專案的文本化匯出流程，目標是讓 GX Works2 / GX Works3 專案可以被 Git 版本控制。
version: 0.3.0
---
...
Plan a text-based export workflow for PLC projects so downstream skills can generate project structure and parameter documentation.
...
- Use `plc-project-normalizer` after raw files are exported.
- Use `plc-doc-generator` after normalized JSON exists.
- Use `plc-parameter-explainer` for CPU/module/network parameter documentation.
- Use `plc-structure-analyzer` for program tree, cross reference, and Mermaid diagrams.
